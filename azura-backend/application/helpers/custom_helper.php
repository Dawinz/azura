...
// Don't redirect API requests - they should return JSON, not redirect
// Check REQUEST_URI directly to avoid any CodeIgniter dependencies
$request_uri = $_SERVER['REQUEST_URI'] ?? '';
if (strpos($request_uri, '/index.php') !== false 
    && strpos($request_uri, '/v1/') === false
    ...
    try {
        ...
        if ($ci && method_exists($ci, 'load')) {
            $ci->load->helper('url');
            // Only redirect if we're not in an API context
            if (strpos($request_uri, '/v1/') === false && strpos($request_uri, '/api/') === false) {
                redirect(current_url());
                exit();
            }
        }
    } catch (Exception $e) {
        // Silently fail if CodeIgniter isn't ready or if this is an API request
    }
}
